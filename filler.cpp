...
template <template <class T> class OrderingStructure> animation filler::fill(FillerConfig &config) {
    ...
    bool visited[config.img.width()][config.img.height()];
    ...
    for (int i = 0; i < config.centers.size(); i++) {
        center cen = config.centers[i];
        point temp = point(cen);
//        temp.x = cen.x;
//        temp.y = cen.y;
//        temp.c = cen;
//
...
//        std::cout << os.peek().x << std::endl;
//        std::cout << os.peek().y << std::endl;
//        std::cout << temp.x << std::endl;
//        std::cout << temp.y << std::endl;
        *config.img.getPixel(cen.x, cen.y) = (*config.pickers[i])(cen);

        visited[cen.x][cen.y] = true;
...
//                        std::cout << "j: " << j << std::endl;
                        int nx = curr.x +  direction[i][0];
                        int ny = curr.y + direction[i][1];

                        if (!visited[nx][ny] && (curr.c.color.dist(*config.img.getPixel(nx, ny)) <= config.tolerance)) {

                            point p(nx, ny, curr.c);
                            os.add(p);


                            *config.img.getPixel(nx, ny) = (*config.pickers[i])(p);
                            visited[nx][ny] = true;
                            os.remove();
                            count++;
